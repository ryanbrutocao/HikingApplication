...
var userDataRef = firebase.database().ref("To_Bring").orderByKey();
userDataRef.once("value").then(function (snapshot) {
  snapshot.forEach(function (childSnapshot) {
    var key = childSnapshot.key;
    var childData = childSnapshot.val();

    item_val = childSnapshot.val().item;
...
//adding the item to the page via jQuery
$("#addItem").on("click", function (event) {
  ...
  database.ref("To_Bring").push(fbItem).then((snap) => {
    const key = snap.key
    dataID = key
    $(itemRemove).attr("data_id", key) // setting the data attribut of the unique key... for hopeful removal targeting later
    console.log(key);
    console.log(dataID);


  })
  $(itemHtml).attr("data_id", dataID)
  $(".hiking-list").append(card);
  $("#newItem").val("");
  $("#newItem").val("");
});
...
$(".hiking-list").on("click", ".item-remove", function (event) {
  ...
  unique = $(event.currentTarget).attr("data_id")
  database.ref("To_Bring").child(unique).remove()
